...
��──────────────────────────
    ...
    /** Returns the full manifest (all plugins, enabled or not). */
    public static function getAll(): array
    {
        return self::$manifest;
    }

    /** Returns true if the given plugin slug is currently enabled. */
    public static function isEnabled(string $slug): bool
    {
        return !empty(self::$manifest[$slug]['enabled']);
    }

    /** Enable a plugin and persist to DB. */
    public static function enable(string $slug): void
    {
        self::syncToDb($slug, true);
    }

    /** Disable a plugin and persist to DB. */
    ...
    /**
     * Purpose : Upsert a plugin record in the plugins table and update in-memory manifest.
     * Inputs  : $slug, $enabled
     * Side effects : DB write, updates self::$manifest[$slug]['enabled']
     */
    private static function syncToDb(string $slug, bool $enabled): void
    {
        $db = Database::getInstance();
