...
<div class="page-header">
  <div><h1 class="page-title">Edit budget</h1><p class="page-sub"><?= htmlspecialchars($budget['title']) ?></p></div>
  <a href="<?= BASE_URL ?>/budgets/<?= $budget['id'] ?>" class="btn">← Back</a>
</div>
...
<div class="form-layout">
  <form method="POST" action="<?= BASE_URL ?>/budgets/<?= $budget['id'] ?>/update">
    <?= Auth::csrfField() ?>
    ...
    <div class="card form-card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="title" name="title" value="<?= htmlspecialchars($budget['title']) ?>" required>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label for="period_type">Period type</label>
          <select id="period_type" name="period_type">
            <option value="weekly"  <?= $budget['period_type'] === 'weekly'  ? 'selected' : '' ?>>Weekly</option>
            <option value="monthly" <?= $budget['period_type'] === 'monthly' ? 'selected' : '' ?>>Monthly</option>
          </select>
        </div>
        <div class="form-group">
          <label for="total_income">Expected income ($)</label>
          <input type="number" id="total_income" name="total_income" step="0.01" min="0" value="<?= htmlspecialchars($budget['total_income']) ?>">
        </div>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Start date</label>
          <input type="date" name="start_date" value="<?= htmlspecialchars($budget['start_date']) ?>" required>
        </div>
        <div class="form-group">
          <label>End date</label>
          <input type="date" name="end_date" value="<?= htmlspecialchars($budget['end_date']) ?>" required>
        </div>
      </div>
      <div class="form-group">
    ...
    <div class="form-actions">
      <a href="<?= BASE_URL ?>/budgets/<?= $budget['id'] ?>" class="btn">Cancel</a>
